...
local Dagger = WCS.RegisterSkill("Dagger")
...
function Dagger:OnStartServer()
	...
	if self.Character.Instance.Humanoid.FloorMaterial == Enum.Material.Air and self.Combo == 5 then
		...
		AnimationObject = Animations.Combats.Dagger[self.Combo]
	end
	...
	if not (self.Character.Instance.Humanoid.FloorMaterial == Enum.Material.Air) then
	...
	self.Maid:GiveTask(Track:GetMarkerReachedSignal("HitPoint"):Connect(function()
		...
		Hitbox:createHitbox({
			...
			elseif self.Character.Instance.Humanoid.FloorMaterial == Enum.Material.Air and self.Combo == 5 then
				FistVFX.new(target, 6):Start(Visuals:GetPlayers(self.Character.Instance))
	...
	local Length = self.Combo == 5 and 1 or Track.Length - 0.5
