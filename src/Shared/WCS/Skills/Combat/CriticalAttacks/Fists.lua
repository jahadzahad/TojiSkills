...
local FistHeavy = WCS.RegisterSkill("FistHeavy")

function FistHeavy:OnStartServer()
	self:ApplyCooldown(30)
	self.Maid = Maid.new()
	local character = self.Character.Instance
	local Track = Animation.GetAnimation(
		character,
		Animations.Combats.Fists.SlideAttack
	)
	Animation.PlayAnimation(character, Animations.Combats.Fists.SlideAttack)
